...
var _skrollr = _interopRequireDefault(require("skrollr"));
...
document.querySelectorAll(".text-slider").forEach(function (slider) {
  var textArray = slider.getAttribute("data-text").split(", ");
  var animatedText = slider.querySelector(".animated-text");
  var currentIndex = 0;
  var slideText = function slideText() {
    var text = textArray[currentIndex];
    animatedText.innerHTML = ""; // Clear existing text

    // Break text into spans for individual letter animation
    text.split("").forEach(function (letter) {
      var span = document.createElement("span");
      span.textContent = letter;
      span.style.display = "inline-block"; // Ensure spans are animatable
    ...
    // Wait before starting the exit animation
    setTimeout(function () {
      return slideOut(spans);
    }, 2000);
  };
  var slideOut = function slideOut(spans) {
    // Elastic Exit Animation
    (0, _animejs["default"])({
      targets: spans,
      translateY: [0, -30],
      // Letters slide downward
      opacity: [1, 0],
      // Fade out
      duration: 1000,
      easing: "easeInElastic(1, 0.75)",
      // Elastic easing for exit
      delay: _animejs["default"].stagger(100, {
        direction: "reverse"
      }),
      // Reverse wave effect
      complete: function complete() {
        currentIndex = (currentIndex + 1) % textArray.length; // Go to next text
        slideText(); // Restart animation
      }
    });
  };
  slideText();
...
},{"animejs":1,"skrollr":22,"smooth-scroll":23}],25:[function(require,module,exports){
"use strict";
...
jsmouseOver.forEach(function (parent) {
  ...
  // Function to get mouse direction (top, right, bottom, or left)
  function getMouseDirection(event, parentRect) {
    var x = event.clientX - parentRect.left;
    var y = event.clientY - parentRect.top;
    var w = parentRect.width;
    var h = parentRect.height;
    var fromTop = y < h / 4;
    var fromBottom = y > 3 * h / 4;
    var fromLeft = x < w / 4;
    var fromRight = x > 3 * w / 4;
  ...
  parent.addEventListener('mouseenter', function (event) {
    ...
    var direction = getMouseDirection(event, parentRect);

    // Reset styles before animation
    linkmouseover.style.transition = 'none';
    linkmouseover.style.opacity = '0';

    // Set initial position based on entry direction
    switch (direction) {
      case 'top':
        linkmouseover.style.transform = 'translate3d(0, -100%, 0) scale(1)';
        break;
      case 'bottom':
        linkmouseover.style.transform = 'translate3d(0, 100%, 0) scale(1)';
        break;
      case 'left':
        linkmouseover.style.transform = 'translate3d(-100%, 0, 0) scale(1)';
        break;
      case 'right':
        linkmouseover.style.transform = 'translate3d(100%, 0, 0) scale(1)';
        break;
      default:
        linkmouseover.style.transform = 'translate3d(0, 0, 0) scale(1)';
        break;
    }

    // Trigger the animation to bring the element into view
    requestId = requestAnimationFrame(function () {
      linkmouseover.style.transition = 'transform 0.4s cubic-bezier(0.165, 0.84, 0.44, 1), opacity 0.4s ease';
      linkmouseover.style.transform = 'translate3d(0, 0, 0) scale(1)';
      linkmouseover.style.opacity = '1'; // Fade in
    });
  });
  ...
  parent.addEventListener('mouseleave', function (event) {
    ...
    // Set the exit direction based on the mouse leave direction
    switch (direction) {
      case 'top':
        linkmouseover.style.transform = 'translate3d(0, -100%, 0) scale(1)';
        break;
    ...
    linkmouseover.style.transition = 'transform 0.4s cubic-bezier(0.165, 0.84, 0.44, 1), opacity 0.4s ease';
    linkmouseover.style.opacity = '0'; // Fade out
  });
});
...
},{"smooth-scroll":23}],38:[function(require,module,exports){
"use strict";

(function () {
  // Function to load an external SVG file asynchronously
  ...
  // Function to inject the SVG into the container and apply fill and stroke colors
  function injectSVG(element) {
    ...
    loadSVG(svgSrc, function (error, svg) {
      ...
      // Apply the fill color if specified
      if (fillColor) {
      ...
      // Apply the stroke color if specified
      if (strokeColor) {
  ...
  // Iterate over all elements with the class 'svg-container' and inject the corresponding SVGs
  document.addEventListener("DOMContentLoaded", function () {
...
},{"./custom/animations":24,"./custom/aos":25,"./custom/buttons":26,"./custom/custom-bootstrap":27,"./custom/dark-mode":28,"./custom/form-validation":29,"./custom/glightbox":30,"./custom/header-sticky":32,"./custom/header-sticky-reverse":31,"./custom/isotope-layout":33,"./custom/jarallax":34,"./custom/mouseover":35,"./custom/preloader":36,"./custom/smooth-scroll":37,"./custom/svgInjector":38,"./custom/topTop":39}]},{},[40]);
